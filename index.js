...
function customAssign(...objects) {
  ...
  for (const i in newObj) {
    if (typeof newObj[i] === "number") {
      newObj[i] = String(newObj[i]);
    }
    if (typeof newObj[i] === "string") {
      newObj[i] = newObj[i].replace(" ", "_");
    }
  }
  return newObj;
}
...
function customObjectFromEntries(entries) {
  ...
  let newObj = Object.fromEntries(mapEntries);
  return newObj;
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  let objFreeze = Object.freeze(objAssign);
...
function filterObjectsByKey(arr, key) {
  ...
  let filterArr = arr.filter((elem) => elem.hasOwnProperty(key));
  return filterArr;
}
